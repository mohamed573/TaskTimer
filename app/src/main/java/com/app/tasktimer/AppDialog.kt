...
// dialog id to select which dialog are we going to display
// Values are stored in a bundle as a key value pair
// setMessage() fun to tell the builder what text to display
// setMessage() is overloaded fun so we can use Int or String
// setPostiveButton and setNegativeButton fun to set the text on the buttons
// dialogInterafce useful in custom though  , because it allows us to call the dialog's cancel function
// which is the button is clicked , if we're allowing our dialog to allow one of a list of options to be chosen
// we can use the keys to retrieve the values
